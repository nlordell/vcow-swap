...
function createGetUrl(options) {
  async function getUrl(req, _signal) {
    ...
    assert(
      protocol === "https" || !req.credentials ||
...
class FetchRequest {
  ...
  get headers() {
    ...
    if (this.#creds) {
      headers["authorization"] = `Basic ${
        encodeBase64(toUtf8Bytes(this.#creds))
      }`;
    }
    if (this.allowGzip) {
  ...
  toString() {
    return `<FetchRequest method=${JSON.stringify(this.method)} url=${
      JSON.stringify(this.url)
    } headers=${JSON.stringify(this.headers)} body=${
  ...
  async #send(attempt, expires, delay, _request, _response) {
    ...
    if (response.statusCode === 301 || response.statusCode === 302) {
      ...
      try {
        ...
        return req.redirect(location).#send(
          attempt + 1,
          expires,
          0,
          _request,
      ...
      // Throttle
      if (
        this.retryFunc == null || (await this.retryFunc(req, response, attempt))
      ) {
    ...
    if (this.processFunc) {
      ...
      try {
        ...
        // Something went wrong during processing; throw a 5xx server error
        if (error.throttle == null || typeof (error.stall) !== "number") {
          response.makeServerError("error in post-processing function", error)
            .assertOk();
        }
        // Throttle
        let delay = this.#throttle.slotInterval *
  ...
  redirect(location) {
    ...
    assert(
      this.method === "GET" && (current !== "https" || target !== "http") &&
        location.match(/^https?:/),
      `unsupported redirect`,
      "UNSUPPORTED_OPERATION",
      {
        operation: `redirect(${this.method} ${JSON.stringify(this.url)} => ${
...
class FetchResponse {
   ...
   */
  ok() {
    return (this.#error.message === "" && this.statusCode >= 200 &&
      this.statusCode < 300);
  }
  /**
...
function formatUnits(value, unit) {
  ...
  return FixedNumber.fromValue(value, decimals, { decimals, width: 512 })
    .toString();
}
/**
...
class Reader {
  ...
  #peekBytes(offset, length, loose) {
    ...
    if (this.#offset + alignedLength > this.#data.length) {
      if (
        this.allowLoose && loose && this.#offset + length <= this.#data.length
      ) {
...
const crypto$1 = typeof globalThis === "object" && "crypto" in globalThis
  ? globalThis.crypto
  : undefined;
...
class SHA512 extends SHA2 {
  ...
  process(view, offset) {
    ...
    for (let i = 16; i < 80; i++) {
      ...
      const s0h = u64$1.rotrSH(W15h, W15l, 1) ^ u64$1.rotrSH(W15h, W15l, 8) ^
        u64$1.shrSH(W15h, W15l, 7);
      const s0l = u64$1.rotrSL(W15h, W15l, 1) ^ u64$1.rotrSL(W15h, W15l, 8) ^
        u64$1.shrSL(W15h, W15l, 7);
      // s1 := (w[i-2] rightrotate 19) xor (w[i-2] rightrotate 61) xor (w[i-2] rightshift 6)
      const W2h = SHA512_W_H[i - 2] | 0;
      const W2l = SHA512_W_L[i - 2] | 0;
      const s1h = u64$1.rotrSH(W2h, W2l, 19) ^ u64$1.rotrBH(W2h, W2l, 61) ^
        u64$1.shrSH(W2h, W2l, 6);
      const s1l = u64$1.rotrSL(W2h, W2l, 19) ^ u64$1.rotrBL(W2h, W2l, 61) ^
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const sigma1h = u64$1.rotrSH(Eh, El, 14) ^ u64$1.rotrSH(Eh, El, 18) ^
        u64$1.rotrBH(Eh, El, 41);
      const sigma1l = u64$1.rotrSL(Eh, El, 14) ^ u64$1.rotrSL(Eh, El, 18) ^
      ...
      const sigma0h = u64$1.rotrSH(Ah, Al, 28) ^ u64$1.rotrBH(Ah, Al, 34) ^
        u64$1.rotrBH(Ah, Al, 39);
      const sigma0l = u64$1.rotrSL(Ah, Al, 28) ^ u64$1.rotrBL(Ah, Al, 34) ^
...
const shiftsL = /* @__PURE__ */ idxL.map((idx, i) =>
  idx.map((j) => shifts[i][j])
);
const shiftsR = /* @__PURE__ */ idxR.map((idx, i) =>
  idx.map((j) => shifts[i][j])
);
...
function scryptInit(password, salt, _opts) {
  ...
  if (
    N <= 1 || (N & (N - 1)) !== 0 || N >= 2 ** (blockSize / 8) || N > 2 ** 32
  ) {
  ...
  function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    try {
      if (typeof sg === "string" || sg instanceof Uint8Array) {
        ...
        typeof sg === "object" && typeof sg.r === "bigint" &&
        typeof sg.s === "bigint"
      ) {
        const { r, s } = sg;
...
async function checkAddress(target, promise) {
  ...
  if (
    result == null || result === "0x0000000000000000000000000000000000000000"
  ) {
    assert(
...
class NumberCoder extends Coder {
  ...
  constructor(size, signed, localName) {
    ...
    super(name, name, localName, false);
    defineProperties(this, { size, signed }, {
      size: "number",
      signed: "boolean",
    });
  }
...
function init() {
  ...
  WHOLE_MAP = new Map();
  let wholes = read_sorted_array().concat(Array_from(WHOLE_VALID)).sort((
    a,
    b,
  ) => a - b); // must be sorted
  wholes.forEach((cp, i) => {
...
function quoted_cp(cp) {
  return (should_escape(cp) ? "" : `${bidi_qq(safe_str_from_cps([cp]))} `) +
    quote_cp(cp);
}
...
function check_label_extension(cps) {
  if (cps.length >= 4 && cps[2] == HYPHEN && cps[3] == HYPHEN) {
    ...
    ); // this can only be ascii so cant be bidi
  }
}
          ...
          for (let i = 1; i < token_count; i++) { // we've already checked the first token
            ...
            if (!cps.is_emoji && CM.has(cps[0])) { // every text token has emoji neighbors, eg. EtEEEtEt...
              ...
              throw error_placement(
                `emoji + combining mark: "${str_from_cps(tokens[i - 1])} + ${
        ...
        if (j - i > NSM_MAX) {
          ...
          throw new Error(
            `excessive non-spacing marks: ${
              bidi_qq(safe_str_from_cps(decomposed.slice(i - 1, j)))
            } (${j - i}/${NSM_MAX})`,
          );
        }
        i = j;
...
class Transaction {
  ...
  set maxFeePerBlobGas(value) {
    this.#maxFeePerBlobGas = (value == null)
      ? null
      : getBigInt(value, "maxFeePerBlobGas");
  }
  /**
...
function _pack(type, value, isArray) {
  ...
  if (match) {
    ...
    assertArgument(
      (!match[2] || match[2] === String(size)) && (size % 8 === 0) &&
        size !== 0 && size <= 256,
      "invalid number type",
      "type",
      type,
...
function lex(text) {
  ...
  const throwError = (message) => {
    const token = (offset < text.length)
...
class ParamType {
  ...
  static from(obj, allowIndexed) {
    ...
    if (
      type === "tuple" || type.startsWith("tuple(" /* fix: ) */) ||
      type.startsWith("(" /* fix: ) */)
    ) {
...
class Interface {
  ...
  #getFunction(key, values, forceUnique) {
    ...
    if (key.indexOf("(") === -1) {
      ...
      if (values) {
        const lastValue = (values.length > 0)
      ...
      if (
        matching.length === 1 && values &&
        values.length !== matching[0].inputs.length
      ) {
        const lastArg = values[values.length - 1];
        if (
          lastArg == null || Array.isArray(lastArg) ||
...
class TransactionResponse {
  ...
  constructor(tx, provider) {
    ...
    this.maxFeePerBlobGas = (tx.maxFeePerBlobGas != null)
      ? tx.maxFeePerBlobGas
      : null;
    this.chainId = tx.chainId;
    this.signature = tx.signature;
    this.accessList = (tx.accessList != null) ? tx.accessList : null;
    this.blobVersionedHashes = (tx.blobVersionedHashes != null)
      ...
      while (nextScan <= blockNumber) {
        ...
        for (let i = 0; i < block.length; i++) {
          ...
          if (tx.from === this.from && tx.nonce === this.nonce) {
            ...
            let reason = "replaced";
            if (
              tx.data === this.data && tx.to === this.to &&
              tx.value === this.value
            ) {
              reason = "repriced";
            } else if (
              tx.data === "0x" && tx.from === tx.to && tx.value === BN_0$2
            ) {
              reason = "cancelled";
            }
...
function buildWrappedFallback(contract) {
  const populateTransaction = async function (overrides) {
    ...
    if (
      iface.fallback && !iface.fallback.payable && iface.receive && !noData &&
      !noValue
    ) {
      assertArgument(
        false,
...
class BaseContract {
  ...
  async off(event, listener) {
    ...
    if (listener) {
      const index = sub.listeners.map(({ listener }) => listener).indexOf(
        listener,
      );
      if (index >= 0) {
...
class AbstractProvider {
  ...
  async getFeeData() {
    ...
    const getFeeDataFunc = async () => {
      ...
      if (block && block.baseFeePerGas) {
        maxPriorityFeePerGas = (priorityFee != null)
          ? priorityFee
          : BigInt("1000000000");
  ...
  async #call(tx, blockTag, attempt) {
    ...
    try {
      ...
      if (
        !this.disableCcipRead && isCallException(error) && error.data &&
        attempt >= 0 && blockTag === "latest" && transaction.to != null &&
  ...
  async #hasSub(event, emitArgs) {
    ...
    if (
      sub.type === "event" && emitArgs && emitArgs.length > 0 &&
...
function parseOffchainLookup(data) {
  ...
  result.errorArgs = "sender,urls,calldata,selector,extraData".split(/,/).map((
    k,
  ) => result[k]);
  return result;
}
    ...
    return await (new Promise((resolve, reject) => {
      ...
      const checkTx = async () => {
        try {
          ...
          if (
            isError(error, "CANCELLED") || isError(error, "BAD_DATA") ||
        ...
        try {
          ...
          this.emit(
            ...
            makeError(
              ...
              "NETWORK_ERROR",
              { event: "initial-network-discovery", info: { error } },
            ),
          );
          await stall$3(1000);
        }
...
function spelunkData(value) {
  ...
  if (
    typeof (value.message) === "string" && value.message.match(/revert/i) &&
  ...
  async fetch(module, params, post) {
    ...
    request.processFunc = async (request, response) => {
      ...
      if (module === "proxy") {
        ...
        if (
          result && result.status == 0 && result.message == "NOTOK" && throttle
        ) {
          this.emit("debug", {
            action: "receiveError",
            id,
...
class SocketSubscriber {
  ...
  start() {
    this.#filterId = this.#provider.send("eth_subscribe", this.filter).then(
      (filterId) => {
...
function _normalize(value) {
  ...
  switch (typeof value) {
    ...
    case "object": {
      ...
      keys.sort();
      return "{" +
        keys.map((k) => `${JSON.stringify(k)}:${_normalize(value[k])}`).join(
          ",",
        ) + "}";
    }
  }
        ...
        if (this.#height === -2) {
          this.#height = Math.ceil(
            getNumber(
              getMedian(
                this.quorum,
                this.#configs.filter((c) => (!c._lastFatalError)).map((c) => ({
                  value: c.blockNumber,
      ...
      const broadcasts = this.#configs.map(
        async ({ provider, weight }, index) => {
          try {
            ...
            results[index] = Object.assign(normalizeResult({ result }), {
              weight,
            });
          } catch (error) {
            results[index] = Object.assign(normalizeResult({ error }), {
              weight,
            });
          }
        },
...
class CTR extends ModeOfOperation {
  constructor(key, initialValue) {
    ...
    __classPrivateFieldSet(
      ...
      "f",
    ); // This will be discarded immediately
  ...
  encrypt(plaintext) {
    ...
    for (let i = 0; i < crypttext.length; i++) {
      ...
      crypttext[i] ^= __classPrivateFieldGet(
        this,
        _CTR_remaining,
        "f",
      )[
...
class Wallet extends BaseWallet {
  ...
  static #fromAccount(account) {
    ...
    if (
      "mnemonic" in account && account.mnemonic &&
